...
void AShotgun::FireShotgun(const TArray<FVector_NetQuantize>& HitTargets)
{
	...
	if (MuzzleFlashSocket)
	{
		...
		//Maps Character hit to total danage
		...
		//Calculate head shot damage by multiplying times hit x HitShotDamage - store in DamageMap
		for (auto HeadShotHitPair : HeadShotHitMap)
		{
			if (HeadShotHitPair.Key)
			{
				if (DamageMap.Contains(HeadShotHitPair.Key)) DamageMap[HeadShotHitPair.Key]+=HeadShotHitPair.Value*HeadShotDamage;
				else HeadShotHitMap.Emplace(HeadShotHitPair.Key, HeadShotHitPair.Value * HeadShotDamage);
		...
		for (auto DamagePair : DamageMap)
		{
			if (DamagePair.Key && InstigatorController)
			{
				...
				{
					UGameplayStatics::ApplyDamage(
						DamagePair.Key, //Charcter that was hit
