...
void ABlasterPlayerController::CheckPing(float DeltaTime)
{
	...
	if (HighPingRunningTime > CheckPingFrequency)
	{
		if (PlayerState)
		{
			//UE_LOG(LogTemp, Warning, TEXT("PlayerState->GetPingInMilliseconds():%d"), (int)PlayerState->GetPingInMilliseconds());
...
void ABlasterPlayerController::SetHUDTime()
{
	...
	if (CountdownInt != SecondsLeft) //时间更新了
	{
...
void ABlasterPlayerController::ClientReportServerTime_Implementation(float TimeOfClientRquest, float TimeServerReceivedClientRequest)
{
	float RoundTripTime = GetWorld()->GetTimeSeconds() - TimeOfClientRquest;
...
{
	if (HasAuthority()) GetWorld()->GetTimeSeconds();
...
{
	BlasterHUD = BlasterHUD == nullptr ? Cast<ABlasterHUD>(GetHUD()) : BlasterHUD;
	/*UE_LOG(LogTemp, Warning, TEXT("Enter Cooldown,Announcement is:%s"), (BlasterHUD->Announcement == nullptr ? TEXT("false") : TEXT("true")));*/
	if (BlasterHUD)
	{
