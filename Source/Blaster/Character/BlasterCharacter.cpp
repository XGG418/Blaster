...
ABlasterCharacter::ABlasterCharacter()
{
	...
	CameraBoom->SetupAttachment(GetMesh()); //绑定到网格上以适应角色下蹲改变胶囊体
...
void ABlasterCharacter::MoveForward(float Value)
{
	...
	if (Controller != nullptr && Value != 0.f)
	{
		...
		//从FRotate创建矩阵，得到单位轴，就是前进的方向
		const FVector Direction(FRotationMatrix(YawRotation).GetUnitAxis(EAxis::X));
		AddMovementInput(Direction, Value); //速度和加速度从角色移动组件中检索
	}
}
...
void ABlasterCharacter::AimOffset(float DeltaTime)
{
	...
	if (Speed == 0.f && !bIsInAir)  //静止站立，没有跳跃
	{
		...
		bUseControllerRotationYaw = true; //之前写成false，在本地控制时会出错，远程代理不会出错
		//原地转向
	...
	//{
	//	//测试结果：在客户端上往下看时，Pitch值在服务器上是从360往下降
	// //原因：在多人游戏中，Pitch值被压缩成了一个无符号数
...
{
	/*UE_LOG(Log_Temp, Warning, TEXT("AO_Yaw : %f"), AO_Yaw);*/
	if (AO_Yaw > 90.f) //右转
	{
		TurningInPlace = ETurningInPlace::ETIP_Right;
	}
	...
	if (TurningInPlace != ETurningInPlace::ETIP_NotTurning) //如果在转
	{
		//将InterpAO_Yaw插值到0
		InterpAO_Yaw = FMath::FInterpTo(InterpAO_Yaw, 0.f, DeltaTime, 4.f);
		AO_Yaw = InterpAO_Yaw;
		if (FMath::Abs(AO_Yaw) < 15.f) //已经转够角度了
		{
			TurningInPlace = ETurningInPlace::ETIP_NotTurning;
			StartingAimRotation = FRotator(0.f, GetBaseAimRotation().Yaw, 0.f); //重新设置瞄准起转角度
		}
	}
}

void ABlasterCharacter::HideCameraIfCharcterClose()
{
